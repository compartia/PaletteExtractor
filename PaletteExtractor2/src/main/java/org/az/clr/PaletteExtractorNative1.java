...
public class PaletteExtractorNative1 implements PaletteExtractor {
    ...
    public PaletteExtractorNative1(final ColorModel cm) {
        this.cm = cm;
    }

    public MapOfLists<ColorTools.RgbColor, Integer> assignColorsToSeeds(
            final RgbColor[] seedsRGB, final List<Integer> argbImage) {
    ...
    public RgbColor[] extract(final int[] argbImage, final int paletteSize) {
        ...
        for (int f = 0; f < paletteSize; f++) {
            ...
            seeds[f] = ColorTools.meanColor(subList);
        }
